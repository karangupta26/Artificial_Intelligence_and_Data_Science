# Practical 3 - Perform inter-process communication between a two child process
...
while True:
    try:
        ...
        if recv=="Stop" or recv=="End":
            sys.exit(0)
        if i == 15:                                                                 # value at which you have to send end message from Child
            send="End"
            print(send)                                                             # stdout tied to pipe: goes to parent's stdin
            sys.stdout.flush()                                                      # make sure it's sent now or else process blocks
            sys.exit()
        else:
            send = 'Child 2 (%d) got: [%s]' % (mypid, recv)
        print(send)                                                                 # stdout tied to pipe: goes to parent's stdin
        sys.stdout.flush()                                                          # make sure it's sent now or else process blocks
        i+=1
    except KeyboardInterrupt:                                                       # key board interrupt
        print('Interrupted')
        try:
            sys.exit(0)
        except SystemExit:
